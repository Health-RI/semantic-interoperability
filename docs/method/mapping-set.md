...
The mapping set versioning is based on the YYYY-MM-DD format, corresponding to its publication date. There will be at most one version released per day.

The mapping records themselves are **append-only**: once a record is published, it is never deleted or edited. If a mapping needs correction or refinement, add a new record that *supersedes* the old one using the `replaces` field. This creates a clear history: consumers can follow the `replaces` chain and use the latest entry for any given mapping.
