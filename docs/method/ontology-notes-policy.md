...
| **CNST** | Constraints & rules    | Plain-language paraphrase of constraints       | paraphrase of rules, invariants, pre/postconditions, multiplicity rationales (no formal syntax) | R1C2 — Spanish Pink — <div style="background-color:#FFC0C0;width:90px;height:22px;border:1px solid #999;text-align:center;line-height:22px;">#FFC0C0</div> |
...
- Do not write formal constraint syntax (e.g., SHACL, OCL) inside CNST notes. These notes contain plain-language descriptions only.
...
- **CNST notes must be anchored.** Anchor to the element governed by the constraint that the note paraphrases.
- The constraint's name appears in parentheses in the CNST prefix so the note can be associated with its complete definition maintained in the SHACL constraints file. The model contains only the readable description; the formal rule lives in that external source of truth.
...
<!-- TODO: Update with links to the contstaints definitions page. -->
