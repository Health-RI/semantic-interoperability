...
*Figure 2: If an external ontology defines `External:Patient` which we map via `hriv:semanticsDefinedBy` to `hrio:Patient`—and `hrio:Patient` derives its semantics from the OntoUML `Patient` concept—then we can interpret `External:Patient` as conveying the same semantics as the `OntoUML Patient` concept.*

## Mapping Properties for Cross-Scheme Alignment

The Health-RI Mapping Vocabulary was created to provide a set of mapping properties designed for expressing alignments between concepts in different concept schemes (e.g. external vocabularies and our Health-RI ontology). The principal properties are: `hriv:semanticsDefinedBy`, `hriv:semanticsDefinedByBroad`, and `hriv:semanticsDefinedByNarrow`.

### Our Strategy: Choosing the Right Mapping Property

- **`hriv:semanticsDefinedBy`** is used when the external concept is fully equivalent in meaning to our reference concept.
  - Each external concept may have **only one** `hriv:semanticsDefinedBy`.
- **`hriv:semanticsDefinedByBroad`** is used when the external concept is **broader** than our reference concept (i.e., it includes our concept and possibly more).
- **`hriv:semanticsDefinedByNarrow`** is used when the external concept is **narrower** than our reference concept (i.e., it captures a more specific notion).

!!! note "Clarifying hriv:semanticsDefinedBy semantics"
    Although `hriv:semanticsDefinedBy` can be treated like a strong semantic alignment **in meaning**, it does **not imply logical equivalence** (such as `owl:equivalentClass`). It is intended to capture a strong semantic alignment **in meaning**, not in logical entailment. This allows semantic interoperability without risking unintended reasoning consequences in OWL-based systems.

In contrast to `hriv:semanticsDefinedBy`, both `hriv:semanticsDefinedByNarrow` and `hriv:semanticsDefinedByBroad` allow **multiple mappings per concept** to express partial or hierarchical semantic overlaps.
...
!!! tip "How to assert an exact match in your OWL file"
    ...
    - `hc:Patient hriv:semanticsDefinedBy <https://w3id.org/health-ri/ontology/v2.0.0#HealthcarePatient> .`
...
!!! tip "How to assert broader or narrower mappings in your OWL file"
    ...
    You may also reference a specific version using the full ontology's URI (e.g., for v2.0.0):
      ...
      - `ghc:Patient hriv:semanticsDefinedByNarrow <https://w3id.org/health-ri/ontology/v2.0.0#VeterinaryPatient> .`
...
*Figure 5: Following the scenario from Figure 4, new intermediate concepts (in green) were added to the Health-RI ontology to bridge semantic gaps and enable the replacement of approximate mappings (`hriv:semanticsDefinedByNarrow`, `hriv:semanticsDefinedByBroad`) with exact ones.*

In cases where an external ontology cannot be mapped via `hriv:semanticsDefinedBy` due to a lack of equivalent concepts, we encourage internal teams and external partners to **contact the Health-RI modeling team**. By collaboratively extending the reference ontology with **missing intermediate concepts**, we support:

- The replacement of `hriv:semanticsDefinedByNarrow` and `hriv:semanticsDefinedByBroad` with `hriv:semanticsDefinedBy`,
